...
for (let click = 0; click < 5; click++) {
  ...
  if (!r.path) { console.log(`  demand ≥${required}: NO PATH`); break; }
  const labels = await getItemLabels(r.path.nodes);
  const ok = r.path.hopCount >= required ? '✓' : '✗ SHORT';
  console.log(`  demand ≥${required}: got ${r.path.hopCount} hops ${ok}  (${Date.now() - t0}ms, explored ${r.stats.explored})`);
